...
This formulation that updates the action-value table after a single step is called TD(0). Another variant is to choose $$G_{s_t, a_t)}$$ using TD0 combined with an $$\epsilon$$-greedy random choice for the action $$a_{t+1}$$. This randomization ensures that optimal values may not be overlooked.
