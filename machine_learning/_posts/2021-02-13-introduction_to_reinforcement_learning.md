...
These are called the Bellman optimality equations. For finite $$\mathcal{A}$$, we have effectively constructed a maximal policy $$\pi$$, picking in state $$s$$ the *greedy* action $$a$$ that maximizes $$Q_\star(s, a)$$ - while for infinite $$\mathcal{A}$$, such a maximal policy may not exist, but can be uniformly approximated by policies $$\pi_\epsilon$$ such that $$Q_{\pi_\epsilon}(s, a) \gt Q_star(s, a) - \epsilon$$ and $$V_{\pi_\epsilon}(s) \gt V_star(s) - \epsilon$$.
