...
In what follows, we assume that rewards are bounded by $$-M \le r_t \le M$$ for all $$t$$, and $$\gamma \lt 1$$. When the number of steps is infinite, then $$r(\overline{\tau})$$ is [uniformly convergent](/machine_learning/rl/math_appendix#convergence-for-return-of-a-trajectory) with respect to all choices of infinite trajectories:
...
Then, using the Bellman equation $$V_\pi(s) = \int_a Q_\pi(s, a) da$$, it is immediate that $$V_{\pi_\epsilon}(s) \gt \underset{a \eps \mathcal{A}, \pi}{sup} \, Q_\pi(s, a) - \epsilon$$, so $$V_\star(s) \gt \underset{a \in \mathcal{A}, \pi}{sup} \, Q_\pi(s, a)$$ for all $$\epsilon \gt 0$$.
